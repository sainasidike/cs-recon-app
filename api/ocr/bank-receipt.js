...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!boundary) return res.status(400).json({ error: '无效的请求格式' });

    const fileBuffer = extractFileFromMultipart(body, boundary);
    if (!fileBuffer) return res.status(400).json({ error: '未找到文件' });
...
function extractFileFromMultipart(body, boundary) {
  ...
  for (const part of parts) {
    ...
    if (headers.includes('filename=')) return part.slice(headerEnd + 4);
  }
  return null;
}
