...
export default function ReconApp() {
  ...
  const [manualMatchFrom, setManualMatchFrom] = useState(null);
  ...
  const handleManualMatch = useCallback((bankEntry, ledgerEntry) => {
    setMatchResults(prev => {
      if (!prev) return prev;
      const newManual = { bank: bankEntry, ledger: ledgerEntry, score: 100, amtDiff: Math.abs(getAmt(bankEntry) - getAmt(ledgerEntry)), daysDiff: 0, manual: true };
      return {
        ...prev,
        exact: [...prev.exact, newManual],
        unmatchedBank: prev.unmatchedBank.filter(b => b.id !== bankEntry.id),
        unmatchedLedger: prev.unmatchedLedger.filter(l => l.id !== ledgerEntry.id),
        matchedCount: prev.matchedCount + 1,
        matchedAmt: prev.matchedAmt + getAmt(bankEntry),
      };
    });
    setManualMatchFrom(null);
  }, []);
    ...
    <div className="rc">
      ...
      {step === 'landing' && (
        <div className="rc-landing">
          ...
          <div className="rc-land-section">
            <h2 className="rc-land-h2">CamScanner 智能财务对账</h2>
            <p className="rc-land-intro">扫描或导入财务单据，AI 自动完成数据提取、智能匹配、差异分析、生成对账报告——全流程无需手动操作。</p>
            <div className="rc-land-divider" />
            <h3 className="rc-land-h3">为什么需要对账？</h3>
            <div className="rc-land-reasons">
              <div className="rc-land-reason"><span className="rc-land-reason-bar" />银行可能扣错手续费</div>
              <div className="rc-land-reason"><span className="rc-land-reason-bar" />录入报销单时多打了一个零</div>
              <div className="rc-land-reason"><span className="rc-land-reason-bar" />货已到但款还在路上，账目暂时对不上</div>
            </div>
            <p className="rc-land-reason-end">错误随时发生，对账就是把它们找出来。</p>
          </div>

          {/* S3: 四步流程 */}
          <div className="rc-land-section gray">
            <h2 className="rc-land-h2">4 步完成，每步都有 AI</h2>
            <div className="rc-land-flow">
              <div className="rc-land-flow-card">
                <div className="rc-land-flow-head"><span className="rc-land-flow-num">1</span>数据采集</div>
                <div className="rc-land-flow-pain">"别让我手动调格式"——银行流水和 ERP 导出的日期、列名永远对不上</div>
                <div className="rc-land-flow-ai">AI 自动识别表头、统一日期格式、清洗脏数据。丢进去就行，不用管哪列是什么。</div>
              </div>
              <div className="rc-land-flow-card">
                <div className="rc-land-flow-head"><span className="rc-land-flow-num">2</span>自动勾对</div>
                <div className="rc-land-flow-pain">"1对1、1对多、多对多都要能配"——备注写着'张三1月货款'，要能匹配到张三的多个订单</div>
                <div className="rc-land-flow-ai">模糊算法 + 语义理解，消化 90% 标准项，把精力留给那 10% 对不上的。</div>
              </div>
              <div className="rc-land-flow-card">
                <div className="rc-land-flow-head"><span className="rc-land-flow-num">3</span>异常识别</div>
                <div className="rc-land-flow-pain">"告诉我为什么对不上"——不只标红，要说原因</div>
                <div className="rc-land-flow-ai">识别重复入账、跨月错位、数字颠倒（123→132）等 10 种典型错误，并给出解释。</div>
              </div>
              <div className="rc-land-flow-card">
                <div className="rc-land-flow-head"><span className="rc-land-flow-num">4</span>输出报告</div>
                <div className="rc-land-flow-pain">"我要一份能直接发给审计的东西"</div>
                <div className="rc-land-flow-ai">生成余额调节表 / 差异汇总报告，可下载 Excel 直接作为审计附件。</div>
              </div>
            </div>
          </div>
          ...
          <div className="rc-land-section">
            ...
            <div className="rc-land-tags">
              <span>银行流水</span><span>台账</span><span>财务记录</span><span>银行凭证</span>
              <span>结算单</span><span>账单</span><span>发票</span><span>结账单</span>
              <span>完税证明</span><span>收据</span>
            </div>
          ...
          <div className="rc-land-section dark">
            <h2 className="rc-land-h2 light">对账只是过程，平账才是目的</h2>
            <div className="rc-land-features">
              <div className="rc-land-feat">
                <div className="rc-land-feat-icon">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                </div>
                <div className="rc-land-feat-body">
                  <div className="rc-land-feat-title">OCR 增强理解</div>
                  <div className="rc-land-feat-desc">拍一张模糊的纸质收据，AI 自动提取金额并匹配系统里的报销单</div>
                </div>
              </div>
              <div className="rc-land-feat">
                <div className="rc-land-feat-icon">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="2"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                </div>
                <div className="rc-land-feat-body">
                  <div className="rc-land-feat-title">追溯链路</div>
                  <div className="rc-land-feat-desc">点击差异项，立刻看到关联的合同、发票、原始凭证，不用再翻文件夹</div>
                </div>
              </div>
              <div className="rc-land-feat">
                <div className="rc-land-feat-icon">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                </div>
                <div className="rc-land-feat-body">
                  <div className="rc-land-feat-title">自动建议调账</div>
                  <div className="rc-land-feat-desc">AI 写好会计分录——"借：财务费用-手续费，贷：银行存款"，复制到财务系统即可平账</div>
                </div>
              </div>
              <div className="rc-land-feat">
                <div className="rc-land-feat-icon">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                </div>
                <div className="rc-land-feat-body">
                  <div className="rc-land-feat-title">智能异常诊断</div>
                  <div className="rc-land-feat-desc">不只标红差异，还告诉你"这 500 元差异疑似银行手续费未入账"</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {step === 'alldocs' && (
        <div className="rc-alldocs">
          {/* Top-level: full header with title + search + actions */}
          {!allDocsFolder ? (
            <>
                ...
                <div className="rc-alldocs-list">
                  <div className="rc-alldocs-folder" onClick={() => setAllDocsFolder('recon')}>
                    ...
                    <div className="rc-alldocs-folder-info">
                      ...
                      <div className="rc-alldocs-folder-meta">{new Date().toLocaleDateString('zh-CN')} | {history.length} 项</div>
                    </div>
                  ...
                  <div className="rc-alldocs-folder">
                    ...
                    <div className="rc-alldocs-folder-info">
                      ...
                      <div className="rc-alldocs-folder-meta">2026/1/13 12:59 | 15 项</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg>
                  </div>
                  ...
                  <div className="rc-alldocs-folder">
                    ...
                    <div className="rc-alldocs-folder-info">
                      ...
                      <div className="rc-alldocs-folder-meta">2025/8/25 15:10 | 2 项</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg>
                  </div>
                  ...
                  <div className="rc-alldocs-folder">
                    ...
                    <div className="rc-alldocs-folder-info">
                      ...
                      <div className="rc-alldocs-folder-meta">2025/8/21 15:34 | 1 项</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ccc" strokeWidth="2"><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg>
                  </div>
                </div>
            ...
            <>
              {/* Level 3: Inside a project folder — show documents */}
              <div className="rc-alldocs-folder-header">
                <button className="rc-alldocs-folder-back-btn" onClick={() => setAllDocsProject(null)}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <span className="rc-alldocs-folder-title">{allDocsProject.company} · {allDocsProject.period}</span>
                <button className="rc-alldocs-folder-more">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg>
                </button>
              </div>
              <div className="rc-alldocs-folder-search">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#999" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <span>搜索文档</span>
              </div>
                      ...
                      {allFiles.map((d, i) => (
                        <div key={i} className="rc-alldocs-filecard" onClick={() => {
                          ...
                          <div className="rc-alldocs-filecard-thumb">
                            ...
                            ) : d.category === 'result' && d.type === 'result_sheet' ? (
                              <div className="rc-alldocs-filecard-icon rc-alldocs-filecard-icon-sheet">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M3 15h18M9 3v18"/></svg>
                              </div>
                            ) : (
                              <div className="rc-alldocs-filecard-icon rc-alldocs-filecard-icon-doc">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#4a90d9" strokeWidth="1.5"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                              </div>
                            )}
                          </div>
                          <div className="rc-alldocs-filecard-info">
                            <div className="rc-alldocs-filecard-name">{d.name}</div>
                            <div className="rc-alldocs-filecard-meta">
                              {allDocsProject.time || ''}
                              {d.category === 'source' && <span className="rc-alldocs-filecard-pages">⊡ 1</span>}
                              {d.category === 'result' && <span className="rc-alldocs-filecard-tag">对账结果</span>}
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            <>
              {/* Level 2: Inside 财务对账 — show project folders */}
              <div className="rc-alldocs-folder-header">
                <button className="rc-alldocs-folder-back-btn" onClick={() => { setAllDocsFolder(null); setAllDocsProject(null); }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2.5"><polyline points="15 18 9 12 15 6"/></svg>
                </button>
                <span className="rc-alldocs-folder-title">财务对账</span>
                <button className="rc-alldocs-folder-more">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg>
                </button>
              </div>
              ...
              <div className="rc-alldocs-body">
                {history.length === 0 ? (
                  ...
                  <div className="rc-alldocs-filelist">
                    {history.map((h) => (
                      <div key={h.id} className="rc-alldocs-filecard" onClick={() => setAllDocsProject(h)}
                        onTouchStart={() => { longPressTimer.current = setTimeout(() => setDeleteConfirmId(h.id), 600); }}
                        onTouchEnd={() => clearTimeout(longPressTimer.current)}
                        onTouchMove={() => clearTimeout(longPressTimer.current)}
                      >
                        <div className="rc-alldocs-filecard-thumb">
                          <div className="rc-alldocs-filecard-icon rc-alldocs-filecard-icon-folder">
                            {h.status === 'pending' ? (
                              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#f5a623" strokeWidth="1.5"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>
                            ) : (
                              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#3DD598" strokeWidth="1.5"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>
                            )}
                          </div>
                        </div>
                        <div className="rc-alldocs-filecard-info">
                          <div className="rc-alldocs-filecard-name">
                            {h.company} · {h.period}
                          ...
                          <div className="rc-alldocs-filecard-meta">
                            {h.time} | {(h.docs || []).length + (h.resultDocs || []).length} 个文档
                          </div>
                        </div>
                      </div>
                    ))}
      ...
      {/* HOME */}
      {step === 'home' && (
        <div className="rc-section" style={{ padding: '16px 20px' }}>
      ...
      {step === 'results' && matchResults && reconData && (
        <div className="rc-section">
          ...
          {activeResultTab === 'unmatched' && (
            <>
              <div className="rc-unmatched-actions">
                <button className="rc-supplement-btn" onClick={() => {
              ...
              {manualMatchFrom && (
                <div className="rc-manual-match-panel">
                  <div className="rc-manual-match-header">
                    <span>选择配对项（{manualMatchFrom.side === 'bank' ? '从企业未达中选' : '从银行未达中选'}）</span>
                    <button className="rc-manual-match-close" onClick={() => setManualMatchFrom(null)}>取消</button>
                  </div>
                  <div className="rc-manual-match-from">
                    <span className="rc-match-tag bank">{manualMatchFrom.side === 'bank' ? '银行' : '企业'}</span>
                    <span>{manualMatchFrom.entry.date}</span>
                    <span className="rc-match-desc">{getDesc(manualMatchFrom.entry)}</span>
                    <span className="rc-um-amt">{getDir(manualMatchFrom.entry) === 'debit' ? `-¥${fmt(getAmt(manualMatchFrom.entry))}` : `+¥${fmt(getAmt(manualMatchFrom.entry))}`}</span>
                  </div>
                  <div className="rc-manual-match-candidates">
                    {(manualMatchFrom.side === 'bank' ? matchResults.unmatchedLedger : matchResults.unmatchedBank).map(item => (
                      <div key={item.id} className="rc-manual-match-candidate" onClick={() => {
                        if (manualMatchFrom.side === 'bank') handleManualMatch(manualMatchFrom.entry, item);
                        else handleManualMatch(item, manualMatchFrom.entry);
                      }}>
                        <span className="rc-um-date">{item.date}</span>
                        <span className="rc-um-desc">{getDesc(item)}</span>
                        <span className={`rc-um-amt ${getDir(item) === 'debit' ? 'out' : 'in'}`}>{getDir(item) === 'debit' ? `-¥${fmt(getAmt(item))}` : `+¥${fmt(getAmt(item))}`}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {!manualMatchFrom && matchResults.unmatchedBank.length > 0 && (<div className="rc-card"><div className="rc-card-title danger">银行未达 ({matchResults.unmatchedBank.length})</div>{matchResults.unmatchedBank.map(b => (
                <div key={b.id} className="rc-unmatched-item">
                  ...
                  <button className="rc-manual-match-btn" onClick={() => setManualMatchFrom({ side: 'bank', entry: b })}>手动匹配</button>
                </div>
              ))}</div>)}
              {!manualMatchFrom && matchResults.unmatchedLedger.length > 0 && (<div className="rc-card"><div className="rc-card-title danger">企业未达 ({matchResults.unmatchedLedger.length})</div>{matchResults.unmatchedLedger.map(l => (
                <div key={l.id} className="rc-unmatched-item">
                  ...
                  <button className="rc-manual-match-btn" onClick={() => setManualMatchFrom({ side: 'ledger', entry: l })}>手动匹配</button>
                </div>
              ))}</div>)}
            </>
          )}
